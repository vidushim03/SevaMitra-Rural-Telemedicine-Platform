...
const Consultations = lazy(() =>
  import("./pages/Consultations").then((m) => ({ default: m.Consultations })),
);
const RecordsPage = lazy(() =>
  import("./pages/Records").then((m) => ({ default: m.RecordsPage })),
);
const PrescriptionsPage = lazy(() =>
  import("./pages/Prescriptions").then((m) => ({
...
const PaymentAnalyticsPage = lazy(() =>
  import("./pages/PaymentAnalytics").then((m) => ({
    default: m.PaymentAnalyticsPage,
  })),
);
...
const SymptomsPage = lazy(() =>
  import("./pages/Symptoms").then((m) => ({ default: m.SymptomsPage })),
);
const PharmacyPage = lazy(() =>
  import("./pages/Pharmacy").then((m) => ({ default: m.PharmacyPage })),
);
const MedicinesPage = lazy(() =>
  import("./pages/Medicines").then((m) => ({ default: m.MedicinesPage })),
);
const VitalsPage = lazy(() =>
  import("./pages/Vitals").then((m) => ({ default: m.VitalsPage })),
);
...
const LanguageToggle = () => {
  ...
  return (
    <button
      ...
      className="flex items-center gap-2 p-2 rounded-full hover:bg-black/10 dark:hover:bg-white/10 transition-colors font-semibold uppercase text-sm"
    >
      <Globe size={20} />
      {language}
    </button>
...
function AccessGuard({ path, children }) {
  const { user } = useAuth();
  if (!user || !ROLE_ACCESS[user.role]?.includes(path)) {
    return (
      <div className="rounded-2xl border bg-card p-6">
        This page is not available for your role. Use the sidebar to navigate
        your portal.
      </div>
    );
  }
...
function ProtectedApp() {
  ...
  const links = baseLinks.map((link) => {
    ...
    const key = link.label.toLowerCase().replace(" ", "");
  ...
  return (
    <div className="flex h-screen w-full bg-slate-50 dark:bg-[#09090b] text-foreground transition-colors duration-300 relative overflow-hidden">
      ...
      <aside
        className={`${isCollapsed ? "w-24" : "w-72"} transition-all duration-300 border-r border-border glass flex flex-col z-10 m-4 rounded-3xl overflow-hidden relative group/sidebar`}
      >
          ...
          {!isCollapsed && (
            <div className="overflow-hidden whitespace-nowrap">
              ...
              <p className="text-xs text-muted-foreground mt-2">
                {user.name} ({t[user.role] || user.role})
              </p>
            </div>
          )}
          {isCollapsed && (
            <div className="mx-auto font-bold text-xl text-primary flex items-center justify-center">
              SM
            </div>
          )}
        ...
        <nav className="flex-1 p-4 flex flex-col gap-2 overflow-y-auto">
          {links.map(({ to, label, icon: Icon }) => {
            ...
            return (
              ...
              >
                <Icon
                  className={`${isActive ? "text-primary" : "text-primary/70"} group-hover:scale-110 transition-transform`}
        ...
        <div
          className={`p-4 border-t border-border/50 flex ${isCollapsed ? "flex-col gap-4 items-center justify-center" : "items-center justify-between"}`}
        >
          <ThemeToggle />
          ...
          >
            <Routes>
              ...
              <Route
                path="/consultations"
                element={
                  <AccessGuard path="/consultations">
                    <Consultations />
                  </AccessGuard>
                }
              />
              ...
              <Route
                path="/prescriptions"
                element={
                  <AccessGuard path="/prescriptions">
                    <PrescriptionsPage />
                  </AccessGuard>
                }
              />
              ...
              <Route
                ...
                element={
                  <AccessGuard path="/payment-analytics">
