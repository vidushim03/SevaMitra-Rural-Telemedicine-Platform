...
export function AdminPage() {
  ...
  if (!user || user.role !== "admin") {
    return (
      <div className="rounded-2xl border bg-card p-6">{t.adminAccessOnly}</div>
    );
  }
  ...
  const tabs = [
    ...
    {
      id: "doctors",
      label: `${t.doctors} (${doctors.length})`,
      icon: Stethoscope,
    },
    {
      id: "patients",
      label: `${t.patients} (${patients.length})`,
      icon: UserCheck,
    },
    {
      id: "appointments",
      label: `${t.appointments} (${data.appointments.length})`,
      icon: Calendar,
    },
    {
      id: "records",
      label: `${t.records} (${data.records.length})`,
      icon: FileText,
    },
    {
      id: "payments",
      label: `${t.payments} (${data.payments.length})`,
      icon: CreditCard,
    },
  ];
        ...
        <div className="space-y-4">
          ...
          {showAddPatient && (
            ...
            >
              ...
              <button className="px-5 py-2 rounded-lg bg-emerald-600 text-white text-sm font-medium">
                {t.addPatient}
              </button>
            </form>
          )}
          ...
          <div className="rounded-2xl border overflow-hidden">
            <table className="w-full text-sm">
              ...
              <tbody>
                {patients.map((pat) => {
                  const apptCount = data.appointments.filter(
                    (a) => a.patientId === pat.id,
                  ).length;
                  const recCount = data.records.filter(
                    (r) => r.patientId === pat.id,
                  ).length;
                  const rxCount = data.prescriptions.filter(
                    (p) => p.patientId === pat.id,
                  ).length;
                  return (
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                ...
                {data.appointments.map((a) => {
                  const patName =
                    data.users.find((u) => u.id === a.patientId)?.name ||
                    a.patientId;
                  const drName =
                    data.users.find((u) => u.id === a.doctorId)?.name ||
                    a.doctorId;
                  return (
                    <tr key={a.id} className="border-t">
                      ...
                      <td className="p-3 flex gap-1">
                        <button
                          ...
                          className="px-2 py-1 rounded bg-emerald-100 dark:bg-emerald-900/30 text-xs"
                        >
                          {t.completeBtn}
                        </button>
                        <button
                          ...
                          className="px-2 py-1 rounded bg-rose-100 dark:bg-rose-900/30 text-xs"
                        >
                          {t.cancel}
                        </button>
                      </td>
          ...
          <div className="rounded-2xl border overflow-hidden">
            <table className="w-full text-sm">
              ...
              <tbody>
                ...
                {data.records.map((r) => {
                  const patName =
                    data.users.find((u) => u.id === r.patientId)?.name ||
                    r.patientId;
                  const drName =
                    data.users.find((u) => u.id === r.doctorId)?.name ||
                    r.doctorId;
                  return (
          ...
          <div className="rounded-2xl border overflow-hidden">
            <table className="w-full text-sm">
              ...
              <tbody>
                ...
                {data.payments.map((p) => {
                  const patName =
                    data.users.find((u) => u.id === p.patientId)?.name ||
                    p.patientId;
                  const drName =
                    data.users.find((u) => u.id === p.doctorId)?.name ||
                    p.doctorId;
                  return (
