...
export const Consultations = () => {
  ...
  const { user } = useAuth();
  const { data, addMessage, setQueueStatus, pendingSyncCount, flushSync } =
    useAppData();
  ...
  const queueItems = useMemo(
    () =>
      data.queue.filter(
        (q) =>
          q.doctorId === user.id &&
          (q.status === "waiting" || q.status === "ongoing"),
      ),
    [data.queue, user],
  );
    ...
    <div className="space-y-6 w-full">
      <div className="rounded-2xl border bg-card p-4">
        ...
        <p className="text-sm text-muted-foreground mt-1">
          {isDoctor ? t.doctorConsultDesc : t.patientConsultDesc}
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-[2fr_1fr] gap-5">
        <div className="min-w-0">
          {isDoctor ? (
            <DoctorDashboard language={language} user={user} />
          ) : (
            <DoctorConsultation language={language} user={user} />
          )}
        </div>
          ...
          <div className="rounded-2xl border bg-card p-4">
            ...
            {pendingSyncCount > 0 && (
              ...
              >
                <RefreshCw
                  size={14}
                  className={isFlushing ? "animate-spin" : ""}
                />
              ...
              {queueItems.map((q) => (
                <div key={q.id} className="rounded-lg border p-3">
                  ...
                  <div className="flex gap-2">
                    <button
                      onClick={() => setQueueStatus(q.id, "ongoing")}
                      className="text-xs px-2 py-1 rounded bg-amber-100 dark:bg-amber-900/40"
                    >
                      {t.markOngoing}
                    </button>
                    <button
                      onClick={() => setQueueStatus(q.id, "finished")}
                      className="text-xs px-2 py-1 rounded bg-emerald-100 dark:bg-emerald-900/40"
                    >
                      {t.finish}
                    </button>
                  </div>
          ...
          <div className="rounded-2xl border bg-card p-4">
            ...
            <div className="h-56 mt-3 rounded-xl border p-2 overflow-auto space-y-2 bg-muted/20">
              ...
              {messages.map((m) => (
                <div
                  key={m.id}
                  className="rounded-lg bg-white dark:bg-zinc-800 p-2 border text-sm"
                >
                  <p className="font-medium text-xs">{m.senderId}</p>
                  <p>{m.text}</p>
                  {m.attachmentName && (
                    <p className="text-xs text-blue-600">
                      {t.attachment} {m.attachmentName}
                    </p>
                  )}
                </div>
              ))}
