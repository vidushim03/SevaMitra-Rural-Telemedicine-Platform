...
export function LoginPage() {
  ...
  const [selectedDoctorId, setSelectedDoctorId] = useState(
    doctorUsers.length > 0 ? doctorUsers[0].id : "",
  );
  ...
  const onSubmit = (ev) => {
    ...
    if (role === "doctor") {
      ...
      if (doc) {
        login({
          name: doc.name,
          email: doc.email || `${doc.id}@demo.com`,
          role: "doctor",
        });
      }
  ...
  return (
    <div className="min-h-screen bg-[radial-gradient(circle_at_20%_20%,#143b8f_0%,#091329_45%,#05070f_100%)] text-white flex items-center justify-center p-6">
      <div className="w-full max-w-md space-y-5">
        <form
          onSubmit={onSubmit}
          className="rounded-3xl border border-white/20 bg-white/10 backdrop-blur-xl p-8 space-y-5 shadow-2xl"
        >
            ...
            <>
              <div className="space-y-2">
                ...
                <input
                  ...
                  className="w-full rounded-xl bg-white/10 border border-white/25 px-4 py-3 outline-none"
                  required
                />
              </div>
          ...
          <button
            type="submit"
            className="w-full rounded-xl bg-blue-500 hover:bg-blue-400 transition py-3 font-semibold"
            disabled={role === "doctor" && !selectedDoctorId}
          >
            {t.continueBtn}
          </button>
        </form>
