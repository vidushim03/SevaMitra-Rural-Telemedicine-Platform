...
export const Dashboard = () => {
  ...
  const stats = isDoctor
    ...
    : [
        ...
        {
          label: isAdmin ? t.revenueCollected : t.totalPaid,
          value: `₹${paidAmount}`,
        },
      ];
  ...
  const quickActions = [
    ...
    ...(isDoctor
      ? [{ to: "/prescriptions", label: t.issuePrescriptions }]
      : []),
    ...
    <div className="space-y-8">
      <div className="rounded-3xl p-6 text-white bg-[linear-gradient(120deg,#0c4a6e_0%,#1e40af_45%,#3b0764_100%)] shadow-2xl">
        <p className="text-white/80 text-sm uppercase">
          {t[user.role] || user.role} {t.portal || "PORTAL"}
        </p>
              ...
              >
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">
                    {t.joined || "Joined:"}{" "}
                    {new Date(q.joinedAt).toLocaleString()}
                  </p>
                </div>
                <span className="text-xs px-2 py-1 rounded-full bg-slate-100 dark:bg-zinc-800 capitalize">
                  {t[q.status] || q.status}
                </span>
              </div>
            ))}
