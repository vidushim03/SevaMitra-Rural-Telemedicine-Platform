...
export function DoctorConsultation({ language, user }) {
  ...
  useEffect(() => {
    ...
    socket.on("call-accepted", async ({ callId }) => {
      ...
      try {
        ...
        const localStream = await service.startLocalStream(
          isVideoEnabled,
          isAudioEnabled,
        );
  ...
  const startConsultation = async (doctorId) => {
    ...
    webrtcService.callDoctor(`doctor_${doctorId}`, currentPatient.id, {
      name: currentPatient.name,
      age: currentPatient.age,
      condition: currentPatient.condition,
    });
      ...
      <div className="max-w-6xl mx-auto">
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
            <Card className="w-96 text-center">
              <CardContent className="p-8">
                ...
                <h3 className="text-xl font-semibold mb-2">
                  {t.incomingDoctorCall}
                </h3>
                <p className="text-gray-600 dark:text-gray-300 mb-6">
                  {incomingCall.patientInfo?.name || t.yourDoctor} would like to
                  start a video consultation with you.
                </p>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {availableDoctors.map((doctor) => (
            <Card
              key={doctor.id}
              className={`transition-all duration-200 hover:shadow-lg ${!doctor.available ? "opacity-60" : "hover:scale-105"}`}
            >
