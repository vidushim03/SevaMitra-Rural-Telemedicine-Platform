  ...
  return (
    <Card className="glass-card overflow-hidden border-none shadow-2xl">
        ...
        <div className="flex justify-between items-center">
          <div>
            <CardTitle className="text-2xl font-bold flex items-center gap-2">
              <FileText size={24} />
              {t.digitalPrescription}
            </CardTitle>
            <p className="opacity-80 text-sm mt-1">{t.patientDoctorLabel.replace('{patientName}', patientName).replace('{doctorName}', doctorName)}</p>
          </div>
          <Badge variant="secondary" className="bg-white/20 text-white border-none backdrop-blur-md">
            {t.consultationFeeLabel.replace('{fee}', consultationFee.toString())}
          </Badge>
        </div>
      </CardHeader>
      ...
      <CardContent className="p-6 space-y-6">
        <div className="flex justify-between items-center">
          <h3 className="font-semibold text-lg">{t.medicationsJanAushadhi}</h3>
              ...
              <div key={med.id} className="p-4 rounded-2xl bg-slate-50 dark:bg-zinc-900 border border-slate-100 dark:border-zinc-800 space-y-3 relative group">
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                  <div className="space-y-1">
                    <label className="text-xs font-medium text-muted-foreground uppercase ml-1">{t.medicineNameLabel}</label>
                    <select
                      value={med.name}
                    ...
                    >
                      <option value="">{t.selectMedicinePlaceholder}</option>
                  ...
                  <div className="space-y-1">
                    <label className="text-xs font-medium text-muted-foreground uppercase ml-1">{t.dosageLabel}</label>
                    <Input 
                      value={med.dosage} 
                  ...
                  <div className="space-y-1">
                    <label className="text-xs font-medium text-muted-foreground uppercase ml-1">{t.frequencyLabel}</label>
                    <Input 
                      value={med.frequency} 
                  ...
                  <div className="space-y-1">
                    <label className="text-xs font-medium text-muted-foreground uppercase ml-1">{t.durationLabel}</label>
                    <Input 
                      value={med.duration} 
                ...
                {priceInfo && (
                  <p className="text-xs text-muted-foreground ml-1">
                    {t.janAushadhiPrice}: ₹{priceInfo.pricePerStrip} {t.pricePerUnit} ({priceInfo.stripSize} {priceInfo.form.toLowerCase()}s)
                  </p>
                )}
                <div className="space-y-1">
                  <label className="text-xs font-medium text-muted-foreground uppercase ml-1">{t.specialInstructionsLabel}</label>
                  <Textarea 
                    value={med.notes} 
        ...
        <div className="pt-4 border-t border-slate-100 dark:border-zinc-800">
          <label className="text-sm font-semibold mb-2 block">{t.generalAdviceDiagnosis}</label>
          <Textarea 
            value={diagnosis}
        ...
        <div className="rounded-2xl bg-slate-50 dark:bg-zinc-900 border border-slate-100 dark:border-zinc-800 p-4 space-y-2">
          <h4 className="font-semibold text-sm">{t.billSummaryGovtRates}</h4>
          <div className="flex justify-between text-sm">
            <span>{t.consultationSpecialtyLabel.replace('{specialty}', specialty)}</span>
      ...
      <CardFooter className="p-6 bg-slate-50 dark:bg-zinc-900/50 flex flex-wrap gap-3">
        <Button onClick={handleTranslate} disabled={isTranslating} variant="secondary" className="rounded-xl transition-all duration-300">
          ...
          {isTranslating ? t.translating : t.translateNotes}
        </Button>
