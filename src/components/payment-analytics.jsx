...
export function PaymentAnalytics() {
  ...
  const doctorMap = useMemo(() => {
    ...
    data.users.forEach((u) => {
      if (u.role === "doctor")
        m.set(u.id, { name: u.name, specialty: u.specialty || "General" });
    });
    return m;
  }, [data.users]);
...
function AdminView({ data, doctorMap, patientMap, t }) {
  ...
  const totalRevenue = useMemo(
    () => paidBills.reduce((s, p) => s + p.amount, 0),
    [paidBills],
  );
  const totalPending = useMemo(
    () => pendingBills.reduce((s, p) => s + p.amount, 0),
    [pendingBills],
  );
  ...
  const revenueByDoctor = useMemo(() => {
    const m = new Map();
    paidBills.forEach((p) =>
      m.set(p.doctorId, (m.get(p.doctorId) || 0) + p.amount),
    );
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
        <KpiCard
          ...
          sub={`${paidBills.length} ${t.paid}`}
        />
        <KpiCard
          icon={<AlertCircle size={20} />}
          label={t.outstandingDues}
          value={`₹${totalPending.toLocaleString("en-IN")}`}
          sub={`${pendingBills.length} ${t.pending}`}
        />
        <KpiCard
          icon={<ReceiptText size={20} />}
          label={t.totalBills}
          value={data.payments.length}
          sub={`₹${(totalRevenue + totalPending).toLocaleString("en-IN")} ${t.bills}`}
        />
        <KpiCard
...
function DoctorView({ data, user, doctorMap, patientMap, t }) {
  const myPaid = useMemo(
    () =>
      data.payments.filter(
        (p) => p.doctorId === user.id && p.status === "paid",
      ),
  ...
  const myPending = useMemo(
    () =>
      data.payments.filter(
        (p) => p.doctorId === user.id && p.status === "pending",
      ),
    [data.payments, user.id],
  );
  const myAll = useMemo(
    () => data.payments.filter((p) => p.doctorId === user.id),
    [data.payments, user.id],
  );

  const myRevenue = useMemo(
    () => myPaid.reduce((s, p) => s + p.amount, 0),
    [myPaid],
  );
  const myPendingAmount = useMemo(
    () => myPending.reduce((s, p) => s + p.amount, 0),
    [myPending],
  );
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
        <KpiCard
          ...
          sub={`${myPaid.length} ${t.paid}`}
        />
        <KpiCard
          icon={<AlertCircle size={20} />}
          label={t.myPending}
          value={`₹${myPendingAmount.toLocaleString("en-IN")}`}
          sub={`${myPending.length} ${t.pending}`}
        />
        <KpiCard
          icon={<ReceiptText size={20} />}
          label={t.myBillsLabel}
          value={myAll.length}
          sub={`${myAll.length} ${t.all}`}
        />
      </div>
      ...
      <div className="rounded-2xl border bg-card p-4">
        <h3 className="font-semibold flex items-center gap-2">
          <AlertCircle size={16} className="text-amber-500" />{" "}
          {t.patientOutstandingDues}
        </h3>
        {patientDues.length === 0 ? (
          <p className="text-sm text-muted-foreground mt-2">
            {t.noPendingDuesFromPatients}
          </p>
        ) : (
          <div className="mt-3 overflow-x-auto">
            <table className="w-full text-sm">
...
function PatientView({ data, user, doctorMap, t }) {
  const myBills = useMemo(
    () => data.payments.filter((p) => p.patientId === user.id),
    [data.payments, user.id],
  );
  const totalBilled = useMemo(
    () => myBills.reduce((s, p) => s + p.amount, 0),
    [myBills],
  );
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
        <KpiCard
          ...
          sub={`${myBills.length} ${t.bills}`}
        />
        <KpiCard
          icon={<CheckCircle size={20} />}
          label={t.totalPaidLabel}
          value={`₹${totalPaid.toLocaleString("en-IN")}`}
          sub={`₹${(totalBilled - totalPaid).toLocaleString("en-IN")} ${t.pending}`}
        />
      </div>
                ...
                {myBills.map((bill) => (
                  <tr key={bill.id} className="border-b last:border-0">
                    <td className="py-2 pr-4 font-medium">
                      {doctorMap.get(bill.doctorId)?.name || bill.doctorId}
                    </td>
                    <td className="py-2 pr-4 text-muted-foreground">
                      {bill.date}
                    </td>
