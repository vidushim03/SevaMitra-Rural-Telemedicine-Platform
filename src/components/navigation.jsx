...
export function Navigation({
  ...
  const patientNavigationItems = [
    ...
    ...(isHealthWorkerMode
      ? [{ id: "health-worker", icon: UserCheck, label: t.healthWorker }]
      : []),
  ];
  ...
  const navigationItems =
    userType === "doctor" ? doctorNavigationItems : patientNavigationItems;
  ...
  return (
    <>
      ...
      {/* Desktop Sidebar */}
      <Card
        className={`hidden md:flex flex-col w-80 h-full p-6 gap-4 border-r-2 ${
          userType === "doctor"
      ...
      {isMenuOpen && (
        <div className="md:hidden fixed inset-0 z-40 bg-black bg-opacity-50">
          <Card
            className={`w-80 h-full p-6 bg-white flex flex-col gap-4 ${
