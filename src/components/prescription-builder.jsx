    ...
    const lineItems = [
      ...
      ...medicines
        ...
        .map((med) => {
          ...
          return {
            description: `${med.name} ${med.dosage} (${priceInfo?.stripSize ?? 10} strips)`,
            quantity: 1,
            unitPrice: price,
            total: price,
          };
    ...
    <Card className="glass-card overflow-hidden border-none shadow-2xl">
      <CardHeader className="premium-gradient text-white p-6">
        <div className="flex justify-between items-center">
          ...
          >
            {t.consultationFeeLabel.replace(
              "{fee}",
      ...
      <CardContent className="p-6 space-y-6">
        ...
        <div className="space-y-4">
          {medicines.map((med) => {
            ...
            return (
              <div
                key={med.id}
                className="p-4 rounded-2xl bg-slate-50 dark:bg-zinc-900 border border-slate-100 dark:border-zinc-800 space-y-3 relative group"
              >
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                  <div className="space-y-1">
                    ...
                    <select
                      value={med.name}
                      onChange={(e) =>
                        handleUpdate(med.id, "name", e.target.value)
                      }
                    ...
                    >
                      ...
                      {MEDICINE_PRICES.map((m) => (
                        <option key={m.name} value={m.name}>
                          {m.name} {m.strength} (₹{m.pricePerStrip})
                        </option>
                      ))}
                    </select>
                  </div>
                  <div className="space-y-1">
                    ...
                    <Input
                      value={med.dosage}
                      onChange={(e) =>
                        handleUpdate(med.id, "dosage", e.target.value)
                      }
                  ...
                  <div className="space-y-1">
                    ...
                    <Input
                      value={med.frequency}
                      onChange={(e) =>
                        handleUpdate(med.id, "frequency", e.target.value)
                      }
                  ...
                  <div className="space-y-1">
                    ...
                    <Input
                      value={med.duration}
                      onChange={(e) =>
                        handleUpdate(med.id, "duration", e.target.value)
                      }
                ...
                {priceInfo && (
                  <p className="text-xs text-muted-foreground ml-1">
                    {t.janAushadhiPrice}: ₹{priceInfo.pricePerStrip}{" "}
                    {t.pricePerUnit} ({priceInfo.stripSize}{" "}
                    {priceInfo.form.toLowerCase()}s)
                  </p>
                )}
                <div className="space-y-1">
                  <label className="text-xs font-medium text-muted-foreground uppercase ml-1">
                  ...
                  <Textarea
                    value={med.notes}
                    onChange={(e) =>
                      handleUpdate(med.id, "notes", e.target.value)
                    }
        ...
        <div className="rounded-2xl bg-slate-50 dark:bg-zinc-900 border border-slate-100 dark:border-zinc-800 p-4 space-y-2">
          ...
          <div className="flex justify-between text-sm">
            <span>
              {t.consultationSpecialtyLabel.replace("{specialty}", specialty)}
            </span>
      ...
      <CardFooter className="p-6 bg-slate-50 dark:bg-zinc-900/50 flex flex-wrap gap-3">
        ...
        {saved ? (
          ...
          <Button
            ...
            variant="outline"
            className="rounded-xl border-primary hover:bg-primary/5"
          >
        ...
        {billCreated ? (
          ...
          <Button
            ...
            className="rounded-xl premium-gradient border-none hover:shadow-xl transition-all"
          >
