    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
        <KpiCard
          ...
          value={metrics.total}
          sub={`${metrics.inProgress} ${t.inProgress}`}
        />
        <KpiCard
          icon={<HeartPulse size={20} />}
          label={t.completedConsultations}
          value={metrics.completed}
          sub={`${metrics.cancelled} ${t.cancelledConsultations}`}
        />
        <KpiCard
          icon={<Store size={20} />}
          label={`${t.paidConsultations} (₹)`}
          value={metrics.revenue.toLocaleString("en-IN")}
          sub={`${metrics.pendingSync} ${t.opsPendingSync}`}
        />
        <KpiCard
          icon={<AlertTriangle size={20} />}
          label={t.urgentCases}
          value={urgencyMix.find((u) => u.name === "emergency")?.value ?? 0}
          sub={t.emergencyTriaged}
        />
      </div>
          ...
          <ul className="mt-3 space-y-2">
            {pharmacyAlerts.map((a, i) => (
              ...
              >
                ...
                <span
                  className={`font-semibold ${a.status === "out_of_stock" ? "text-red-600" : "text-amber-600"}`}
                >
                  {a.status === "out_of_stock" ? t.outOfStock : t.lowStock} (
                  {a.quantity})
                </span>
              </li>
            ))}
