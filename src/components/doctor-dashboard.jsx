...
export function DoctorDashboard({ language, user }) {
  ...
  const waiting = myQueue.filter(
    (q) => q.status === "waiting" || q.status === "ongoing",
  ).length;
  ...
  const todaysPatients = myQueue
    .map((q) => {
      ...
      const patientName =
        data.users.find((u) => u.id === q.patientId)?.name || q.patientId;
      return {
        id: q.id,
      ...
      <div className="min-h-screen bg-black relative">
        ...
        <div className="absolute top-4 left-4 bg-black bg-opacity-70 text-white p-4 rounded-lg min-w-64">
          ...
          <div className="space-y-1 text-sm">
            <div>
              <span className="text-gray-300">{t.patientLabel}</span>{" "}
              {currentPatient?.name}
            </div>
            <div>
              <span className="text-gray-300">{t.ageLabel}</span>{" "}
              {currentPatient?.age} {t.years}
            </div>
            <div>
              <span className="text-gray-300">{t.conditionLabel}</span>{" "}
              ...
              <div className="space-y-2 mb-6 text-left">
                <div className="bg-gray-50 dark:bg-zinc-800 p-3 rounded-lg space-y-1">
                  <p>
                    ...
                    {incomingCall.patientInfo.name}
                  </p>
                  <p>
                    <span className="font-medium text-gray-600 dark:text-gray-300">
                      {t.ageLabel}
                    </span>{" "}
                    {incomingCall.patientInfo.age} {t.years}
                  </p>
                  <p>
                    <span className="font-medium text-gray-600 dark:text-gray-300">
                      {t.conditionLabel}
                    </span>{" "}
                    {incomingCall.patientInfo.condition}
                  </p>
                </div>
              </div>
          ...
          <div className="flex justify-between items-center py-4">
            <div>
              ...
              <p className="text-gray-600 dark:text-gray-300">
                {t.welcomeBack.replace("{name}", currentDoctor.name)}
              </p>
            </div>
            <div className="flex items-center gap-4">
              ...
              >
                <div
                  className={`w-2 h-2 rounded-full mr-2 ${connectionStatus === "connected" ? "bg-green-50 dark:bg-green-900/200 animate-pulse" : "bg-red-500"}`}
                ></div>
                {connectionStatus === "connected"
                  ? t.availableForCalls
                  : t.connectionIssue}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {selectedTab === "dashboard" && (
          <div className="space-y-6">
              ...
              <Card className="border-yellow-200 bg-yellow-50 dark:bg-yellow-900/20">
                <CardContent className="p-4">
                  <div className="flex items-center gap-2 text-yellow-600">
                    ...
                    <span>
                      {t.connectionStatusLabel} {connectionStatus}
                    </span>
                ...
                <CardContent className="p-6">
                  <div className="flex items-center">
                    ...
                    <div className="ml-4">
                      ...
                      <p className="text-2xl font-bold text-gray-900 dark:text-gray-100">
                        {todaysStats.avgConsultationTime}m
                      </p>
                    </div>
                  </div>
                </CardContent>
            ...
            {/* Active Calls */}
            {consultationCalls.some(
              (call) => call.status === "ongoing" || call.status === "incoming",
            ) && (
              <Card>
                <CardHeader>
                          ...
                          <div className="flex items-center">
                            ...
                            <div>
                              ...
                              <p className="text-sm text-gray-600 dark:text-gray-300">
                                {call.status === "ongoing"
                                  ? `${t.duration || "Duration"}: ${call.duration}m`
                                  : call.status === "incoming"
                                    ? t.incomingCall || "Incoming Call"
              ...
              <CardContent>
                <div className="space-y-4">
                  {todaysPatients.map((patient) => (
                    <div
                      key={patient.id}
                      className="flex flex-wrap items-center justify-between p-4 border rounded-lg hover:bg-gray-50 dark:hover:bg-zinc-800 gap-4"
                    >
                    ...
                    .map((patient) => (
                      ...
                      >
                        ...
                        {selectedPatient?.id === patient.id && (
                          <Badge className="bg-blue-100 dark:bg-blue-900/40">
                            {t.selected}
                          </Badge>
                        )}
                      </div>
                  ...
                  <div className="md:col-span-2 space-y-6">
                    <Card>
                      ...
                      <CardContent className="space-y-2">
                        ...
                        <p>
                          <span className="font-medium">{t.genderLabel}</span>{" "}
                          {selectedPatient.gender === "M" ? t.male : t.female}
                        </p>
                        <p>
                        ...
                        <p>
                          <span className="font-medium">{t.symptomsLabel}</span>{" "}
                          {selectedPatient.symptoms}
                        </p>
                        <p>
                          <span className="font-medium">{t.lastVisit}:</span>{" "}
                          {selectedPatient.lastVisit || t.notAvailable}
                        </p>
        ...
        {selectedTab === "records" && (
          <Card>
            ...
            <CardContent>
              <p className="text-gray-600 dark:text-gray-300">
                {t.accessPatientRecords ||
                  "Access comprehensive patient records and consultation history"}
              </p>
            </CardContent>
          </Card>
        )}
